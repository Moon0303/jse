...
/*
 ...
 - C 언어인 경구 숫자, 문자 계열만 Call By Value 이고
   문열은 Char *  형이므로 Call By Address라고 한다.
  - Call By Value 인 경우 값을 전달하면 값을 전송한 원본 변수는 변형이 안된다.
    단지 , stack 간의 값이 복사된다.
  -Call By Reference 의 경우 참조값 (Hash Code) 를 전달한 객체는
   자신의 참조값이 절달 됨으로써 값의 변화가 발생할 수 있다. 
 */
...
/*
 ...
 - 문자열이 같으면 멤버변수나 지역변수는 같은 참조값 (Hash Code) 을 가지고 있다
 - 그리고 둘의 구분은 영영 없어진다
 - 그러나 새로운 문자열이 지역변수에 대입되면
 - 또 다른 참조값 (Hash Code)가 발생되면서 멤버 변수와 구분되어 지게 된다.  
 */
