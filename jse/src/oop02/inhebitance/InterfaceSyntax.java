...
/*
 ...
 - 추상 메소드를 사용하는 이유는 앞으로 추가되거나 구현
   기능의 설계 역할을 하며 실제 기능은 구현하지 않고
   메소드의 프로토타입만 구현하는 것을 맗마.
 
 - 추상 메소드는 건축물에서 구조를 이루는 철근과 같고 설계도와 같은 역할을 함
 
 - 어떤 컴포너트나 클래스의 이용 메소드를 공개할 목적으로 사용됨
 ...
 - 인터페이스는객체를 만들수 없음 . 따라서 반드시 그 인터페이스를 구현한
   클래스의 객체를 받아 사용해야함.
 ...
 - 인터페이스를 이용하면 인터페이스를 구현하는 클래스들은 메소드의 형태가
   ...
   이것은 마치 MySQL등으로 개발된 프로그램을 Oracle 용으로 변경시에
   JDBC 드라이버만 변경하면 프로그램이 작동하는 것과 같은 이치
 
 - 인터페이스는 기능을 구현해서 가지고 있느 ㄴ것이 아니라
   구현되어야 하는 구조를 가지고 있음.
   이런 이유 때문에 컴포넌트의 동일성이 향상됨
 */
 ...
 * 인터페이스 참조
 ...
 - 각 클래스의 멤버변수에 접급하는 경우는 그 클래스 타입으로 형변환을 해주어야함.
